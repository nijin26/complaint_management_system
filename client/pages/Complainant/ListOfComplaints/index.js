...
const ListOfComplaints = () => {
    ...
    if (address) {
      const fetchComplaints = async () => {
        const q = query(
          ...
          where("complainantWalletAddress", "==", address)
        );
  ...
  const viewHandler = async (complaintData) => {
    ...
    const date1 = new Date(decryptedData.complaintCreatedAt).getTime(); // Complaint Created Time
    const date2 = new Date().getTime(); // Today
    const diffTime = date2 - date1; // Difference in timestamps
    const diffDays = Math.round(diffTime / (1000 * 60 * 60 * 24)); // One day in milliseconds
  ...
  const handleForward = () => {
    console.log("Upload complaint details to the respective magistrate");
  };
            ...
            <div className="flex flex-wrap">
              <div className="w-full md:w-1/2 [&>p]:my-2">
                ...
                <p>
                  ...
                  {new Date(selectedComplaint.complaintCreatedAt).toUTCString()}
                </p>
                <p>
                  <span className="font-bold">Number of Days Passed:</span>{" "}
                  {selectedComplaint.daysPassed}
                  <span
                    className="text-red-500 cursor-pointer mx-2 font-bold"
                    onClick={handleForward}
                  >
                    (Forward Report to District Magistrate)
                  </span>
                </p>
                <p>
                ...
                <p>
                  ...
                  {new Date(selectedComplaint.dateAndTime).toUTCString()}
                </p>
                <p>
